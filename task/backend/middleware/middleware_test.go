...
func TestCoordinatingTaskService(t *testing.T) {
	...
	if ex.CurrentID() != scheduler.ID(task.ID) {
		t.Fatalf("task given to scheduler not the same as task created. expected: %v, got: %v", task.ID, ex.CurrentID())
	}
...
func TestCoordinatingTaskService_ForceRun(t *testing.T) {
	...
	if influxdb.ID(ex.CurrentID()) != task.ID {
		t.Fatalf("expected task ID passed to scheduler to match create task ID %v, got: %v", task.ID, ex.CurrentID())
	}
