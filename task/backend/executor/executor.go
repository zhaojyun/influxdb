...
func (e *Executor) Execute(ctx context.Context, id scheduler.ID, scheduledAt time.Time, runAt time.Time) error {
	_, err := e.PromisedExecute(ctx, id, scheduledAt, runAt)
	return err
}
...
func (e *Executor) PromisedExecute(ctx context.Context, id scheduler.ID, scheduledAt time.Time, runAt time.Time) (Promise, error) {
	iid := influxdb.ID(id)
	// create a run
	p, err := e.createRun(ctx, iid, scheduledAt, runAt)
	if err != nil {
		return nil, err
	}
...
func (e *Executor) createRun(ctx context.Context, id influxdb.ID, scheduledAt time.Time, runAt time.Time) (*promise, error) {
	r, err := e.tcs.CreateRun(ctx, id, scheduledAt.UTC(), runAt.UTC())
	if err != nil {
		return nil, err
	}
